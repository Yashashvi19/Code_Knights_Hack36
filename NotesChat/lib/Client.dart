        ...
        child: Column(
          children:[
           ...
           RaisedButton(onPressed: (){
             print("clicked");
             try{
               createSocketConnection(UserStruct(room.text,name.text));
               Navigator.push(
                   context,
                   new MaterialPageRoute(
                       builder: (__) => new Chat(myObject:socket,userName:name.text)));
               // init();
               }
               catch(e){
              print("Error");
...
createSocketConnection(UserStruct user) {
  ...
  socket.onConnect((_) {
    print('connect');
    socket.emit("join room", jsonEncode(UserStruct(room.text,name.text)));
